...
class IssueDirectory extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const routes = <_RouteItem>[
      _RouteItem(
        name:
            '(fixed) #69388 Asset variant detection should only work on directory names that are parse-able by the framework (2x, 3x)',
        page: AssetVariantDetection69388(),
      ),
      _RouteItem(
        name:
            '(fixed) #21326 Assets with Space in file names loads only default resolution',
